...
- `environment.json` from the E2E images is now distro-neutral: `debian_packages` is renamed `system_packages` and `container.debian_snapshot` is renamed `container.snapshot`. Each image lists its distro packages once in `KWIN_MCP_SYSTEM_PACKAGES`; its package-manager step installs that list, and the recorder reports each package's version through `dpkg-query`, `pacman`, or `rpm`.
...
- `accessibility_tree`, `find_ui_elements`, and every element-addressed tool reported all KWrite 26.08 elements as `@ unavailable (no-kwin-window)` on Fedora 44, openSUSE Tumbleweed, and Arch Linux (KWin 6.7.5). KWrite 26.08 names its AT-SPI frame `kwin-mcp-scroll.txt  — KWrite`, with two spaces before the dash, while KWin shows the caption `kwin-mcp-scroll.txt — KWrite` with one, so the frame-to-window caption check rejected the only candidate window. The check now collapses whitespace runs on both sides before comparing. A different document or a different application still does not match, and a bare prefix is still rejected.
...
- Importing `kwin_mcp` failed on systems without libei installed even when no EIS input functionality was used: the shared library was loaded eagerly at import time. It is now loaded lazily on first use, so the server, CLI, and linting, type-checking and packaging environments work without the native library. When `session_start` or `session_connect` reaches for the missing library, the load failure now surfaces through the existing optional-backend path, before any EIS connection is requested from KWin, so `session_start` reports `No input backend available` with an install hint instead of raising an `OSError` that left the started KWin session running. The README Debian/Ubuntu install commands now include `libei1`.
- `keyboard_type_unicode` pasted with Ctrl+V into every app, including terminals (#53). Konsole pastes on Ctrl+Shift+V and passes an unbound Ctrl+V to the program as `^V` (VLNEXT, or a shell's quoted-insert), so the call reported failure and left a `^V` behind that changed the next key: in Konsole running `cat`, the following Return arrived as a literal CR, and in zsh a later bracketed paste ran with its escape sequence quoted. No key sent afterwards can cancel that state without being inserted itself, so the chord is now chosen before anything is pressed, from KWin's active window: Ctrl+Shift+V for terminals known to paste on it (Konsole, Yakuake, GNOME Terminal, GNOME Console, kitty, Alacritty, WezTerm, foot, Ghostty, st, Xfce Terminal, Tilix, Terminator), Ctrl+V for other apps. Recognized terminals with no clipboard paste chord (xterm and urxvt, whose Shift+Insert pastes the primary selection) and windows KWin cannot identify now get a failure result without the clipboard being touched or any key pressed; a terminal whose window class is not recognized still gets Ctrl+V. Typing into Konsole now delivers the text exactly, and a failed call leaves the next key unaltered.
- `session_start` reported `No input backend available` on some libei builds, even though KWin exposed EIS, so every mouse, keyboard, and touch tool was unusable. Reproduced on Fedora 44 and openSUSE Tumbleweed aarch64 (KWin 6.7.5, libei 1.6.0). libei declares `void ei_dispatch(struct ei *ei)`, but kwin-mcp bound it with an `int` return type and stopped the EIS handshake whenever that "value" was negative. A void function leaves whatever the return register held; on those builds it was negative on every call, so the handshake ended before any device was added. `ei_dispatch` is now bound as void and its result ignored; a server disconnect is still reported through `EI_EVENT_DISCONNECT`.
- Processes started by `session_start` and `launch_app` inherited the server's stdin, which over MCP stdio is the JSON-RPC request stream. A launched program that reads stdin, such as a shell script or `cat`, consumed request lines, and those tool calls never got a response. The compositor wrapper and every `launch_app` process, in virtual and live sessions, now get `/dev/null` as stdin.
