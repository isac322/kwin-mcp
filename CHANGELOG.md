...
- Arch Linux end-to-end image `docker/e2e-arch.Dockerfile`. It runs the unchanged `tests/e2e` suite, including every MCP tool such as `mouse_click`, `keyboard_type`, and `accessibility_tree`, against Arch's much newer KWin, libei, Qt, and Mesa, with the same venv, user, entrypoint, and artifacts as the Debian image. The base image digest is pinned and pacman reads a dated Arch Linux Archive snapshot. The E2E workflow gains an `archlinux-amd64` job that fails the workflow on any test failure; there is no arm64 job because no current Arch-family arm64 base image exists. `scripts/run-e2e-docker.sh --distro archlinux` runs it locally.
