...
- `accessibility_tree`, `find_ui_elements`, `wait_for_element` and `list_windows` reuse one long-lived AT-SPI2 worker per session bus (`python -m kwin_mcp.accessibility --serve`, newline-delimited JSON over stdin/stdout) instead of starting a fresh interpreter and importing PyGObject/Atspi on every call. In the E2E image (KWin 6.3.6, 2 CPUs, kcalc open, 231-element tree), interleaved runs measured warm medians of 412 → 323 ms for `accessibility_tree`, 375 → 289 ms for `find_ui_elements` and 65 → 1.6 ms for `list_windows` (median of three rounds' warm medians); the first call on a new worker costs about as much as one call did before. The worker records the D-Bus address it was started with and is replaced when the session's address changes, when it has exited, and on every `session_start` and `session_connect`, so a session that crashed and was replaced without `session_stop` never answers for the new one. A timeout, worker exit or worker-side exception discards the worker and retries once on a fresh one, as before; `session_stop` stops it (killing it after 2 seconds if it does not exit on EOF). It uses `subprocess`, not `multiprocessing`, so it starts no `multiprocessing.resource_tracker` helper and never re-imports the caller's `__main__` module.
