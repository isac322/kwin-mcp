...
| `test_lazy_libei.py` | `kwin_mcp.input` executes without opening libei and loads it once on first use; a missing `libei.so.1` becomes the optional-backend error naming `libei1` and is retried on the next call, and `session_start` then reports `No input backend available` with that reason while the session stays stoppable, KWin exits on `session_stop`, and no socket descriptor outlives it. |
